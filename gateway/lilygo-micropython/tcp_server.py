...
# NDJSON clients need an explicit frame terminator. Legacy clients that search
# for the JSON status remain compatible because the payload is unchanged.
ACK = b'{"status":"OK"}\n'
...
class TCPServer:
    ...
    async def restart(self):
        """Recreate the listener without disturbing device/MQTT state."""
        self.running = False
        ...
        gc.collect()
        await self.start()
        self.logger.warning("TCP server recovered by supervisor")
