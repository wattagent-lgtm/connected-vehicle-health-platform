...
from network_manager import interface, ready, is_ap_mode, station_count
...
class DashboardData:
    ...
    def network(self):
        connected = ready(self.wlan)
        config = self.wlan.ifconfig()
        rssi = None
        if connected and not is_ap_mode():
            try:
                rssi = self.wlan.status("rssi")
            except Exception:
                pass
        return {"wifi_status": "HOSTING" if connected and is_ap_mode()
                else ("CONNECTED" if connected else "DISCONNECTED"),
                "wifi_mode": "AP" if is_ap_mode() else "STA",
                "wifi_clients": station_count(self.wlan),
