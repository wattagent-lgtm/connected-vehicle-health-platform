...
from network_manager import (interface, ready, is_ap_mode, configure_ap)


async def _port_probe(ip, port, http=False):
    reader = None
    writer = None
    try:
        reader, writer = await asyncio.wait_for_ms(
            asyncio.open_connection(ip, port), HTTP_HEALTH_TIMEOUT_MS)
        if http:
...
async def _recover_wifi(wlan, log):
    if is_ap_mode():
        log.warning("WiFi AP inactive; attempting recovery")
        try:
            configure_ap(wlan)
            if ready(wlan):
                log.info("WiFi AP recovered: " + wlan.ifconfig()[0])
                return True
        except Exception as exc:
            log.error("WiFi AP recovery: %s" % exc)
        return False
...
async def _service_supervisor(wlan, tcp, web, log):
    http_failures = 0
    ...
    while True:
        ...
        if elapsed >= HTTP_HEALTH_INTERVAL_SECONDS:
            elapsed = 0
            ip = wlan.ifconfig()[0]
            if await _port_probe(ip, HTTP_PORT, True):
                http_failures = 0
            else:
                http_failures += 1
...
async def start_gateway():
    ...
    wlan = interface()
    asyncio.create_task(_service_supervisor(wlan, tcp, web, log))
