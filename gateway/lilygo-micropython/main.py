...
from network_manager import interface
...
def _fail_safe_reset():
    """Reset immediately; retain a WDT fallback if reset unexpectedly returns."""
    try:
    ...
    try:
        ...
        print("machine.reset failed:", reset_error)
    # machine.reset() normally never returns. If it does, the watchdog above
    # will restore the board instead of leaving WiFi alive with no services.
    while True:
        time.sleep_ms(500)
